package com.liferay.traning.registration.web.portlet.action;
...
import com.liferay.traning.registration.web.constants.MVCCommandNames;
import com.liferay.traning.registration.web.exception.*;
...
public class AddUserMVCActionCommand extends BaseMVCActionCommand {
    ...
    protected void validateAddress(String address, String address2) throws PortalException{
        if(Validator.isNull(address)){
            throw new UserAddressException.MustNotBeNull();
        }
        if(!Validator.isAlphanumericName(address) && !Validator.isAlphanumericName(address2)) {
            throw new UserAddressException.MustBeAlphanumeric();
        }
    ...
    protected void validateZip(String zip) throws PortalException{
        if(Validator.isNull(zip)) {
            throw new UserZipCodeException.MustNotBeNull();
        }
        if(zip.length() != 5 || !Validator.isDigit(zip)) {
            throw new UserZipCodeException.MustBeValidate();
        }
    }
    ...
    public static boolean isValidPassword(String password) {
        ...
        return hasLowerChar && hasUpperChar && hasNumChar && hasSpeChar;
    }
